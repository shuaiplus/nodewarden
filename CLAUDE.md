...
- Official web signup uses `/identity/accounts/register/send-verification-email` + `/finish`. Send a Cloudflare Email (`EMAIL` binding + `EMAIL_FROM`); return an empty JSON string so the client waits for the inbox. `/finish` requires the emailed register-verify JWT.
