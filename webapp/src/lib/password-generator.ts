...
export function normalizeGeneratorSettings(value: unknown): GeneratorSettings {
  ...
  const rsaLengths: SshKeyOptions['rsaLength'][] = [1024, 2048, 3072, 4096];
...
export function estimateStrength(mode: GeneratorMode, value: string): number {
  if (mode === 'username' || mode === 'email' || mode === 'sshKey') return 0;
  if (mode === 'pin') return value.length >= 10 ? 4 : value.length >= 8 ? 3 : value.length >= 6 ? 2 : 1;
  if (mode === 'passphrase') return Math.min(4, Math.max(1, Math.floor(value.split(/[-_. ]/).filter(Boolean).length / 2)));
